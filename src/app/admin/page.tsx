...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async (s: string) => {
    try {
      const res = await fetch(`/api/admin/waitlist?secret=${encodeURIComponent(s)}`);
  ...
  const exportCSV = () => {
    window.open(`/api/admin/waitlist?secret=${encodeURIComponent(secret)}&format=csv`, "_blank");
  };
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0b", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
        <form onSubmit={handleLogin} style={{ ...cardStyle, width: "100%", maxWidth: 360 }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: "white", marginBottom: 16, fontFamily: "Outfit, sans-serif" }}>Admin Login</h1>
          <input
            type="password"
            value={secret}
            onChange={e => setSecret(e.target.value)}
            placeholder="Admin secret"
            style={{ width: "100%", height: 44, padding: "0 14px", borderRadius: 8, background: "#19191d", border: "1px solid rgba(255,255,255,0.1)", color: "white", fontSize: 14, outline: "none", fontFamily: "inherit", boxSizing: "border-box", marginBottom: 12 }}
          />
